...
async function structuredJson({ schema, schemaName, messages }) {
  ...
  if (provider === "openai") return structuredJsonOpenAI({ schema, schemaName, messages, apiKey: cfg.apiKey, model: cfg.model });
  if (provider === "anthropic") return structuredJsonAnthropic({ schema, schemaName, messages, apiKey: cfg.apiKey, model: cfg.model });
  if (provider === "ollama") return structuredJsonOllama({ schema, schemaName, messages, model: cfg.model, host: cfg.host });
  throw new Error("Aucun provider IA configuré. Configure OPENAI_API_KEY ou ANTHROPIC_API_KEY, ou OLLAMA_MODEL (Ollama local).");
}
